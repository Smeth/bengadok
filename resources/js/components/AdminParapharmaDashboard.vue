...
const creditProgressPct = computed(() => {
    const prepayes = props.kpis.credits_prepayes_total;
    if (prepayes <= 0) {
        return 0;
    }
    return Math.min(
        100,
        Math.round(
            (props.kpis.credits_consommes_total / prepayes) * 100,
        ),
    );
});
...
function setMois(value: string) {
    ...
    router.get(dashboard(), { mois: value, tab: 'parapharma' }, { preserveState: true });
}

function marquerPaye() {
    payForm.post('/dashboard/commission/payee', {
        preserveScroll: true,
    });
}
            ...
            >
                ...
                <div class="mt-3">
                    ...
                    <p class="mt-1 text-xs font-medium text-[#6B21A8]">
                        {{ kpis.credits_utilises }} consommés sur la période
                        (1→{{ config.periode_jour_fin }}) ·
                        {{ kpis.credits_prepayes_total }} prépayés (cumul) ·
                        {{ kpis.credits_consommes_total }} consommés (cumul)
                    </p>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-4">
                ...
                >
                    ...
                    <ul class="space-y-2 text-sm text-gray-800">
                        ...
                        <li class="flex justify-between">
                            <span>Valeur consommée ({{ config.credit_prix_unitaire_xaf }} XAF/cr.)</span>
                            <span class="font-bold"
                        ...
                        <li class="flex justify-between">
                            <span>Commandes médicaments éligibles</span>
                            <span class="font-bold">{{
                ...
                >
                    ...
                    <p class="text-sm text-gray-700">
                        <strong>Crédits :</strong> la pharmacie achète des
                        crédits en avance (min.
                        {{ config.credit_minimum_achat }}, à
                        {{ formatXaf(config.credit_prix_unitaire_xaf) }}
                        XAF l’unité). Chaque commande
                        <em>médicaments</em> passée via BengaDok d’un montant ≥
                        {{ formatXaf(config.credit_seuil_medicament_xaf) }}
                        XAF consomme <strong>1 crédit</strong>.
                        <br />
                        <strong>Commission :</strong>
                        {{ config.commission_percent }}&nbsp;% sur le CA des
                        ventes <em>parapharmacie</em> uniquement, à régler avant
                        le {{ config.commission_jour_echeance }} du mois.
                    </p>
                </div>
            </div>
        </div>
