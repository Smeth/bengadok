...
type TypePharmacie = { id: number; designation: string; heurs?: { ouverture: string; fermeture: string } };
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: props.pharmacie.designation, href: `/pharmacies/${props.pharmacie.id}` },
];
...
const createdUsername = computed(() => (page.props.flash as { createdUsername?: string })?.createdUsername);

const typeJour = computed(() => props.types.find((t) => t.designation?.toLowerCase().includes('jour')));
const typeNuit = computed(() => props.types.find((t) => t.designation?.toLowerCase().includes('nuit')));
...
function toggleGarde() {
    router.patch(`/pharmacies/${props.pharmacie.id}/toggle-garde`, {}, { preserveScroll: true });
}
...
function confirmResetPassword() {
    ...
    router.patch(`/pharmacies/${props.pharmacie.id}/users/${userForAction.value.id}/reset-password`, {}, {
        preserveScroll: true,
        onSuccess: () => { showResetPasswordModal.value = false; userForAction.value = null; },
    });
}
...
function confirmRemoveUser() {
    if (!userForAction.value) return;
    router.delete(`/pharmacies/${props.pharmacie.id}/users/${userForAction.value.id}`, {
        preserveScroll: true,
        onSuccess: () => { showRemoveUserModal.value = false; userForAction.value = null; },
    });
}
...
function normalizeErrors(e: unknown): Record<string, string> {
    ...
    const errs = (raw.errors as Record<string, string | string[]> | undefined) ?? raw;
    const out: Record<string, string> = {};
    for (const [k, v] of Object.entries(errs)) {
        const msg = Array.isArray(v) ? v[0] : v;
        if (typeof msg === 'string') {
            out[k] = msg.includes('identifiants') || msg.includes('credentials')
...
function creerUtilisateur() {
    if (formUser.value.passwordMode === 'manual' && !formUser.value.password.trim()) {
        errors.value = { password: 'Veuillez saisir un mot de passe (min. 8 caractères).' };
        return;
    }
    router.post(`/pharmacies/${props.pharmacie.id}/users`, {
        ...
        preserveScroll: true,
        onSuccess: () => { modalCreateUser.value = false; },
        onError: (e) => { errors.value = normalizeErrors(e); },
    });
}
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            <Button variant="link" class="w-fit -ml-2" as-child>
                <Link href="/pharmacies" class="flex items-center gap-2 text-[#459cd1]">
            ...
            <div class="rounded-xl border border-white/80 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/95">
                <div class="mb-6 flex items-start justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold text-foreground">{{ pharmacie.designation }}</h1>
                    ...
                    <Button class="bg-[#459cd1] text-white hover:bg-[#3a8ab8]" @click="openEditModal">
                ...
                <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <!-- Coordonnées -->
                    <div class="rounded-lg border border-sky-100 bg-sky-50/50 dark:border-sky-900/30 dark:bg-sky-900/10">
                        <h3 class="rounded-t-lg bg-sky-100 px-4 py-2 font-semibold text-sky-800 dark:bg-sky-900/50 dark:text-sky-200">
                            Coordonnées
                        </h3>
                        <div class="space-y-3 p-4">
                            ...
                            <p v-if="pharmacie.email" class="flex items-center gap-2 text-sm">
                    ...
                    <div class="rounded-lg border border-sky-100 bg-sky-50/50 dark:border-sky-900/30 dark:bg-sky-900/10">
                        ...
                        <div class="space-y-3 p-4">
                            <p class="flex items-center gap-2 text-sm">
                                ...
                                <span class="text-muted-foreground">Ouverture :</span> {{ pharmacie.heurs?.ouverture ?? '-' }}
                            </p>
                            <p class="flex items-center gap-2 text-sm">
                                <Clock class="size-4 shrink-0 text-sky-600" />
                                <span class="text-muted-foreground">Fermeture :</span> {{ pharmacie.heurs?.fermeture ?? '-' }}
                            </p>
                            <Button
                                v-if="pharmacie.de_garde"
                                variant="destructive"
                    ...
                    <div class="rounded-lg border border-sky-100 bg-sky-50/50 dark:border-sky-900/30 dark:bg-sky-900/10">
                        ...
                        <div class="grid gap-2 p-4 text-sm">
                            <p><span class="text-muted-foreground">Nom :</span> {{ pharmacie.proprio_nom || '-' }}</p>
                            <p><span class="text-muted-foreground">Email :</span> {{ pharmacie.proprio_email || '-' }}</p>
                            <p><span class="text-muted-foreground">Téléphone :</span> {{ pharmacie.proprio_tel || '-' }}</p>
                        </div>
                    </div>
                </div>

                <!-- Utilisateurs -->
                <div class="mt-6 rounded-lg border border-sky-100 dark:border-sky-900/30">
                    <div class="flex items-center justify-between rounded-t-lg bg-sky-100 px-4 py-2 dark:bg-sky-900/50">
                        <h3 class="flex items-center gap-2 font-semibold text-sky-800 dark:text-sky-200">
                            <Users class="size-4" />
                            Utilisateurs
                        </h3>
                        <Button size="sm" class="bg-[#459cd1] text-white hover:bg-[#3a8ab8]" @click="ouvrirCreateUser">
                            <Plus class="mr-2 size-4" />
                ...
                <form class="space-y-6" @submit.prevent="submitEdit">
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="space-y-2 sm:col-span-2">
                            <Label for="edit-designation">Nom de la pharmacie</Label>
                            <Input id="edit-designation" v-model="form.designation" />
                        ...
                        <div class="space-y-2">
                            ...
                            <Input id="edit-telephone" v-model="form.telephone" />
                        </div>
                        <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex gap-4">
                                <label
                                    ...
                                    :class="form.type_pharmacie_id === (typeJour?.id?.toString() ?? '') ? 'border-amber-400 bg-amber-50' : 'border-input'"
                                >
                                    <input v-model="form.type_pharmacie_id" type="radio" :value="typeJour?.id?.toString() ?? ''" />
                                    Pharmacie de jour
                                </label>
                                <label
                                    class="flex flex-1 cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm"
                                    :class="form.type_pharmacie_id === (typeNuit?.id?.toString() ?? '') ? 'border-blue-400 bg-blue-50' : 'border-input'"
                                >
                                    <input v-model="form.type_pharmacie_id" type="radio" :value="typeNuit?.id?.toString() ?? ''" />
                                    Pharmacie de nuit
                                </label>
                            </div>
                        </div>
                        <div class="space-y-2">
                            <Label for="edit-heure_ouverture">Heure d'ouverture</Label>
                            <Input id="edit-heure_ouverture" v-model="form.heure_ouverture" type="time" />
                        </div>
                        <div class="space-y-2">
                            <Label for="edit-heure_fermeture">Heure de fermeture</Label>
                            <Input id="edit-heure_fermeture" v-model="form.heure_fermeture" type="time" />
                        </div>
                    </div>

                    <div class="space-y-2">
                        <Label class="text-muted-foreground">Propriétaire</Label>
                        <div class="grid gap-4 sm:grid-cols-3">
                            <div class="space-y-2">
                                <Label for="edit-proprio_nom">Nom</Label>
                                <Input id="edit-proprio_nom" v-model="form.proprio_nom" />
                            </div>
                            <div class="space-y-2">
                                <Label for="edit-proprio_email">Email</Label>
                                <Input id="edit-proprio_email" v-model="form.proprio_email" type="email" />
                            </div>
                            <div class="space-y-2">
                                <Label for="edit-proprio_tel">Téléphone</Label>
                                <Input id="edit-proprio_tel" v-model="form.proprio_tel" />
                            </div>
                        </div>
                    </div>
        ...
        <Dialog :open="modalCreateUser" @update:open="modalCreateUser = $event">
            <DialogContent class="max-h-[85vh] max-w-[min(32rem,calc(100vw-2rem))] overflow-y-auto overflow-x-hidden">
                <DialogHeader>
                    <DialogTitle class="flex items-center gap-2">
                ...
                <form class="space-y-6" @submit.prevent="creerUtilisateur">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex gap-4">
                            <label
                                ...
                                :class="formUser.role === 'gerant' ? 'border-[#459cd1] bg-sky-50' : 'border-input hover:bg-muted/30'"
                            >
                            ...
                            <label
                                class="flex flex-1 cursor-pointer flex-col items-center gap-2 rounded-lg border-2 p-4 transition-colors"
                                :class="formUser.role === 'vendeur' ? 'border-[#459cd1] bg-sky-50' : 'border-input hover:bg-muted/30'"
                            >
                    ...
                    <div class="rounded-lg bg-muted/50 p-4 space-y-4">
                        <h4 class="font-medium text-muted-foreground">Identifiants</h4>
                        <div class="space-y-2">
                            ...
                            <p class="text-xs text-[#459cd1]">
                                L'aperçu affiche l'identifiant estimé (basé sur le prochain ID). L'identifiant final sera confirmé dans le message de succès après la création.
                            </p>
                        </div>
                        <div class="space-y-3">
                            ...
                            <div class="flex gap-4">
                                <label
                                    class="flex flex-1 cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm"
                                    :class="formUser.passwordMode === 'auto' ? 'border-[#459cd1] bg-sky-50' : 'border-input'"
                                >
                                    <input v-model="formUser.passwordMode" type="radio" value="auto" class="sr-only" />
                                    Générer automatiquement
                                </label>
                                <label
                                    class="flex flex-1 cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm"
                                    :class="formUser.passwordMode === 'manual' ? 'border-[#459cd1] bg-sky-50' : 'border-input'"
                                >
                                    <input v-model="formUser.passwordMode" type="radio" value="manual" class="sr-only" />
                                    Saisir manuellement
                                </label>
                            </div>
                            <div v-if="formUser.passwordMode === 'auto'" class="flex gap-2">
                                <Input
                            ...
                            <div v-else class="relative">
                                ...
                                <button
                                    ...
                                    :aria-label="showPasswordManual ? 'Masquer le mot de passe' : 'Afficher le mot de passe'"
                                    @click="showPasswordManual = !showPasswordManual"
                                >
                                    <EyeOff v-if="showPasswordManual" class="size-4" />
                                    <Eye v-else class="size-4" />
                                </button>
                            </div>
                            <p v-if="errors.password" class="text-sm text-red-600">{{ errors.password }}</p>
                            <p class="text-xs text-[#459cd1]">
                                {{ formUser.passwordMode === 'auto' ? "L'utilisateur devra changer ce mot de passe à la première connexion" : 'Choisissez un mot de passe sécurisé à transmettre à l\'utilisateur' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="rounded-lg bg-sky-50 p-4 dark:bg-sky-950/30">
                        ...
                        <p class="text-sm text-muted-foreground">
                            Mot de passe : <span class="font-mono">{{ formUser.password }}</span>
                        </p>
                        <Button
                            type="button"
                            variant="outline"
                    ...
                    <DialogFooter class="gap-2">
                        <Button type="button" variant="destructive" @click="modalCreateUser = false">
                            Annuler
                        </Button>
                        <Button type="submit" class="bg-[#459cd1] text-white hover:bg-[#3a8ab8]">
                            Créer l'utilisateur
        ...
        <ConfirmModal
            ...
            title="Réinitialiser le mot de passe"
            :description="userForAction ? `Réinitialiser le mot de passe de ${userForAction.name} ? Un nouveau mot de passe sera généré.` : ''"
        ...
        <ConfirmModal
            ...
            :description="userForAction ? `Retirer ${userForAction.name} de cette pharmacie ? L'utilisateur ne pourra plus y accéder.` : ''"
