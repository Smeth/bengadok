...
const pharmaciesDeLaZone = computed(() =>
    zoneSelectionnee.value
        ? props.pharmacies.filter(
              (p) => p.zone?.id === zoneSelectionnee.value!.id,
          )
        : [],
);
...
const pharmaciesFiltrees = computed(() => {
    ...
    if (filtreType.value === 'jour') {
        liste = liste.filter((p) =>
            p.type_pharmacie?.designation?.toLowerCase().includes('jour'),
        );
    } else if (filtreType.value === 'nuit') {
        liste = liste.filter((p) =>
            p.type_pharmacie?.designation?.toLowerCase().includes('nuit'),
        );
    ...
    if (ordonnanceFile.value) {
        ...
        if (fieldApplies('montant_livraison_id') && montantLivraisonId.value)
            formData.append(
                ...
                String(montantLivraisonId.value),
            );
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <!-- Backdrop -->
        <div
            class="flex flex-1 items-start justify-center p-4"
            style="
                background: linear-gradient(
                    135deg,
                    rgba(91, 176, 52, 0.12) 0%,
                    rgba(52, 176, 199, 0.12) 100%
                );
            "
        >
            <!-- Modal card -->
            <div class="w-full max-w-2xl rounded-2xl bg-white shadow-2xl my-2">
                ...
                <div class="max-h-[75vh] overflow-y-auto px-6 py-5 space-y-5">
                    ...
                    <div class="rounded-xl border border-gray-200 p-4">
                        ...
                        <template v-else>
                            ...
                            >
                                ...
                                <p
                                    v-if="!pharmaciesFiltrees.length"
                    ...
                    <div class="rounded-xl border border-gray-200 p-4">
                        ...
                        <!-- Total commande -->
                        <div
                            class="mt-3 flex items-baseline gap-2 border-t pt-3"
                        >
                            <span class="text-sm font-semibold text-gray-700"
                                >Total montant commande :</span
                            >
                            <span class="text-2xl font-bold text-gray-900">{{
                                totalCommande.toFixed(1)
                            }}</span>
                ...
                <!-- ── Pied de page ── -->
                <div
                    class="flex items-center justify-center gap-4 border-t px-6 py-4"
                >
                    <button
                        type="button"
                        @click="annuler"
