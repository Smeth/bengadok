...
const tabClassActive = 'rounded-lg px-4 py-2 text-sm font-medium bg-[#459cd1] text-white shadow-sm';
</script>
        ...
        >
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex flex-wrap items-center gap-4">
                    <Button variant="link" class="w-fit -ml-2 h-auto p-0" as-child>
                        <Link href="/clients" class="flex items-center gap-2 text-[#459cd1] hover:underline">
                            <ArrowLeft class="size-4 shrink-0" />
                            Retour à la liste
                        </Link>
                    </Button>
                    <h1 class="text-2xl font-bold tracking-tight text-foreground">Profil Client</h1>
                </div>
                <Button
                    type="button"
            ...
            <div class="flex flex-wrap gap-2 border-b border-slate-200/80 pb-3 dark:border-white/10">
                <Link href="/clients" :class="tabClassInactive">Liste des clients</Link>
                <Link href="/clients/doublons" :class="tabClassInactive">Gestion des doublons Clients</Link>
            ...
            <div class="grid gap-6 lg:grid-cols-12 lg:items-start">
                ...
                >
                    <div class="mb-6 flex flex-col items-center text-center sm:flex-row sm:items-start sm:text-left">
                        <div
                        ...
                        <div class="min-w-0 flex-1">
                            <h2 class="text-xl font-bold text-foreground">{{ nomComplet() }}</h2>
                    ...
                    <div class="space-y-4 text-sm">
                        <div class="flex gap-3">
                            <Phone class="mt-0.5 size-4 shrink-0 text-[#459cd1]" />
                            <div>
                                <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Téléphone</p>
                                <p class="font-medium text-foreground">{{ telAffiche(client.tel) }}</p>
                                <p v-if="client.tel_secondaire" class="mt-1 text-muted-foreground">
                                    Secondaire : {{ telAffiche(client.tel_secondaire) }}
                                </p>
                            </div>
                        </div>
                        <div class="flex gap-3">
                            <MapPin class="mt-0.5 size-4 shrink-0 text-[#459cd1]" />
                            <div>
                                <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Adresse</p>
                                <p class="text-foreground">{{ client.adresse || '—' }}</p>
                            </div>
                        </div>
                        <div class="flex gap-3">
                            <MapPin class="mt-0.5 size-4 shrink-0 text-[#459cd1]" />
                            <div>
                                <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Zone / Arrondissement</p>
                                <p class="font-medium text-[#459cd1]">
                                    {{ client.zone ? `${client.zone}, Brazzaville` : '—' }}
                                </p>
                            </div>
                        </div>
                        <div class="grid gap-3 border-t border-slate-100 pt-4 dark:border-white/10 sm:grid-cols-2">
                            <div class="flex gap-2">
                                <Calendar class="mt-0.5 size-4 shrink-0 text-muted-foreground" />
                                <div>
                                    <p class="text-xs text-muted-foreground">Client depuis</p>
                                    <p class="font-semibold text-foreground">{{ client.client_depuis || '—' }}</p>
                                </div>
                            </div>
                            <div class="flex gap-2">
                                <Calendar class="mt-0.5 size-4 shrink-0 text-muted-foreground" />
                                <div>
                                    <p class="text-xs text-muted-foreground">Dernière commande</p>
                                    <p class="font-semibold text-foreground">{{ client.derniere_commande || '—' }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-slate-100 pt-4 dark:border-white/10">
                            ...
                            <div v-if="client.niches_labels?.length" class="flex flex-wrap gap-2">
                                <span
                ...
                <div class="flex flex-col gap-6 lg:col-span-8">
                    ...
                    <div class="grid gap-4 sm:grid-cols-3">
                        ...
                        >
                            ...
                            <p class="mt-2 text-3xl font-black tabular-nums text-foreground">{{ client.nb_commandes }}</p>
                        </div>
                        <div
                            class="rounded-2xl border border-slate-200/90 bg-white p-5 text-center shadow-sm dark:border-white/10 dark:bg-white/[0.97]"
                        >
                            <p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Total dépensé</p>
                            <p class="mt-2 text-2xl font-black tabular-nums text-foreground">
                                {{ Number(client.total_depense).toLocaleString('fr-FR') }}
                                <span class="text-base font-bold text-emerald-600">xaf</span>
                            </p>
                        </div>
                        <div
                            class="rounded-2xl border border-slate-200/90 bg-white p-5 text-center shadow-sm dark:border-white/10 dark:bg-white/[0.97]"
                        >
                            ...
                            <p class="mt-2 text-2xl font-black tabular-nums text-foreground">
                                {{ Number(client.panier_moyen).toLocaleString('fr-FR') }}
                                <span class="text-base font-bold text-emerald-600">xaf</span>
                            </p>
                        </div>
                    </div>
                    ...
                    >
                        <h3 class="mb-5 flex items-center gap-2 text-lg font-bold text-foreground">
                        ...
                        <div class="mb-6 grid gap-4 sm:grid-cols-2">
                            ...
                            >
                                <p class="text-3xl font-black tabular-nums text-[#459cd1]">{{ client.pour_soi }}</p>
                                <p class="mt-1 text-sm font-medium text-foreground">Commandes pour soi</p>
                                <p class="mt-1 text-xs text-muted-foreground">{{ client.pct_soi }}% du total</p>
                            </div>
                            <div
                                class="rounded-xl border-2 border-[#459cd1]/40 bg-sky-50/30 p-5 text-center dark:bg-sky-950/20"
                            >
                                <p class="text-3xl font-black tabular-nums text-[#459cd1]">{{ client.pour_tiers }}</p>
                                <p class="mt-1 text-sm font-medium text-foreground">Commandes pour tiers</p>
                                <p class="mt-1 text-xs text-muted-foreground">{{ client.pct_tiers }}% du total</p>
                            </div>
                        </div>

                        <div class="space-y-5 border-t border-slate-100 pt-5 dark:border-white/10">
                            <div>
                                <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                                    Catégories des tiers
                                </p>
                                <div v-if="client.categories_tiers?.length" class="flex flex-wrap gap-2">
                                    <span
                            ...
                            <div>
                                ...
                                <span
                                    v-if="client.tiers_frequent && client.tiers_frequent !== '-'"
                            ...
                            <div>
                                ...
                                <div v-if="client.medicaments_frequents?.length" class="flex flex-wrap gap-2">
                                    <span
