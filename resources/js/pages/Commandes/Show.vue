...
const isAgent = computed(
    () =>
        roles.value.includes('agent_call_center') ||
        roles.value.includes('admin') ||
        roles.value.includes('super_admin'),
);
const isVendeurOuGerant = computed(
    () => roles.value.includes('vendeur') || roles.value.includes('gerant'),
);
const peutModifier = computed(
    () =>
        isAgent.value &&
        ['nouvelle', 'en_attente'].includes(props.commande.status),
);
const peutRenvoyer = computed(
    () => isAgent.value && props.commande.status === 'en_attente',
);
const peutValiderRetrait = computed(
    () =>
        isVendeurOuGerant.value &&
        props.commande.status_pharmacie === 'valide_a_preparer',
);
const peutValiderDispo = computed(
    () =>
        isVendeurOuGerant.value &&
        props.commande.status_pharmacie === 'nouvelle',
);

const peutAssignerLivreur = computed(
    () =>
        isAgent.value &&
        ['validee', 'a_preparer', 'retiree'].includes(props.commande.status),
);
...
const pharmacieRefusee = computed(
    () => props.commande.pharmacieRefusee ?? props.commande.pharmacie_refusee,
);
...
const produitsManquants = computed(() => {
    ...
    return produits.filter((p) => {
        ...
        const qteConf =
            ...
            (pivot.status === 'indisponible' ? 0 : qte);
...
const peutRenvoyerPartiel = computed(
    () =>
        peutRenvoyer.value &&
        !props.commande.parent_id &&
        produitsManquants.value.length > 0,
);

const lignesValidation = ref<
    Record<number, { status: string; quantite_confirmee: number }>
>({});
function initLignes() {
    const produits = props.commande?.produits ?? [];
    ...
    produits.forEach((p) => {
        ...
        const qteConf =
            pivot.quantite_confirmee ?? (status === 'indisponible' ? 0 : qte);
    ...
    const lignes = produits.map((p) => {
        ...
        return {
            ...
            quantite_confirmee:
                status === 'indisponible'
                    ? 0
                    : Math.min(Math.max(0, qteConf), p.pivot?.quantite ?? 0),
        };
    });
    router.post(`/dok-pharma/${props.commande.id}/valider`, { lignes });
}

const motifsAnnulationOptions = computed(
    () => page.props.motifs_annulation ?? [],
);
...
const lignesPartiel = ref<
    Record<number, { selected: boolean; quantite: number }>
>({});
...
function renvoyerPharmaciePartiel() {
    ...
    router.post(
        `/agent/commande/${props.commande.id}/renvoyer-pharmacie-partiel`,
        {
            pharmacie_id: phId,
            lignes,
        },
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-1 flex-col gap-6 p-4">
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    >
                        ...
                        <option
                            v-if="commande.status_pharmacie === 'livre'"
                            value="retiree"
                        >
                            Livrée
                        </option>
                    ...
                    <Link
                        v-if="peutModifier"
                        :href="`/commandes/${commande.id}/edit`"
                    >
                ...
                <Dialog
                    :open="modalAnnulation"
                    @update:open="modalAnnulation = $event"
                >
                    <DialogContent class="sm:max-w-md">
                        ...
                        <p class="text-sm text-muted-foreground mb-4">
                            Veuillez indiquer le motif de l'annulation de cette
                            commande.
                        </p>
                        <select
                ...
                <Dialog
                    :open="modalRenvoyerPartiel"
                    @update:open="modalRenvoyerPartiel = $event"
                >
                    <DialogContent class="sm:max-w-lg">
                        <DialogHeader>
                            <DialogTitle
                                >Renvoyer les produits manquants à une 2ème
                                pharmacie</DialogTitle
                            >
                        </DialogHeader>
                        <p class="text-sm text-muted-foreground mb-4">
                            Sélectionnez les produits à renvoyer et la pharmacie
                            de destination.
                        </p>
                        <div class="space-y-3 mb-4">
                            ...
                            >
                                ...
                                <Input
                                    v-model.number="
                                        lignesPartiel[p.id].quantite
                                    "
                                    type="number"
                                    :min="1"
                                    :max="
                                        (p.pivot?.quantite ?? 0) -
                                        (p.pivot?.quantite_confirmee ?? 0)
                                    "
                                    class="w-16"
                                />
                            </div>
                        ...
                        <div class="mb-4">
                            <label class="mb-2 block text-sm font-medium"
                                >Pharmacie de destination</label
                            >
                            <select
                        ...
                        <DialogFooter>
                            <Button
                                variant="outline"
                                @click="modalRenvoyerPartiel = false"
                            >
                                Annuler
                            </Button>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <div
                    v-if="isAgent && commande.status === 'en_attente'"
                    class="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4"
                >
                    <h3 class="mb-3 font-semibold">Validation client</h3>
                    <p class="mb-3 text-sm text-muted-foreground">
                        Le client doit avoir validé le coût total (produits +
                        livraison) avant de valider la commande.
                    </p>
                ...
                <div class="rounded-xl border p-4">
                    ...
                    <p
                        v-if="pharmacieRefusee"
                        class="mt-2 text-sm text-amber-600"
                    >
                        Pharmacie refusée précédente :
                        {{ pharmacieRefusee.designation }}
                    </p>
                </div>
            </div>

            <div
                v-if="peutValiderDispo"
                class="rounded-xl border border-amber-200 bg-amber-50/50 p-4"
            >
                <h3 class="mb-3 font-semibold">
                    Vérifier disponibilité des médicaments
                </h3>
                <p class="mb-4 text-sm text-muted-foreground">
                    Pour chaque produit : indiquez le statut et précisez la
                    quantité disponible de votre côté.
                </p>
                <div class="space-y-3">
                    <div
                    ...
                    >
                        <span class="flex-1 font-medium"
                            >{{ p.designation }} {{ p.dosage ?? '' }}</span
                        >
                        <span class="text-muted-foreground"
                            >Demandé : {{ p.pivot.quantite }}</span
                        >
                        <select
                        ...
                        >
                            <Input
                                v-model.number="
                                    lignesValidation[p.id].quantite_confirmee
                                "
                                type="number"
                                :min="0"
            ...
            <div class="rounded-xl border p-4">
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="py-2">
                                ...
                                <span
                                    v-if="
                                        p.pivot.status === 'partiel' &&
                                        p.pivot.quantite_confirmee
                                    "
                                >
                ...
                <p class="mt-3 font-semibold">
                    Total :
                    {{ Number(commande.prix_total).toLocaleString('fr-FR') }}
                    XAF
                </p>
                <div
                    v-if="commande.enfants?.length"
                    class="mt-4 rounded-lg border border-amber-200 bg-amber-50/30 p-4"
                >
                    <h4 class="mb-2 font-semibold text-amber-800">
                        Produits renvoyés à une 2ème pharmacie
                    </h4>
                    <div
                    ...
                    >
                        <p class="font-medium text-amber-700">
                            {{ enf.pharmacie?.designation }} ({{ enf.numero }})
                        </p>
                        <ul
                            class="mt-1 list-inside list-disc text-muted-foreground"
                        >
                            <li
                                v-for="(prod, idx) in enf.produits ?? []"
                                :key="idx"
                            >
                                {{
                                    (
