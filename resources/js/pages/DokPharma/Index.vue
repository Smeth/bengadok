...
const props = defineProps<{
    ...
    stats: { nouvelles: number; en_attente: number; a_preparer: number; livrees: number };
    onglet: string;
}>();
...
const confirmModal = ref<{ open: boolean; cmd: Commande | null }>({ open: false, cmd: null });
...
function confirmerAchat() {
    ...
    confirmModal.value = { open: false, cmd: null };
    router.post(`/dok-pharma/${id}/valider-retrait`, {}, { preserveScroll: true });
}
...
function openOrdonnance(cmd: Commande) {
    ordModal.value = { open: true, url: cmd.ordonnance_url ?? '', numero: cmd.numero };
    zoom.value = 100;
}
function closeOrdonnance() { ordModal.value.open = false; }
    ...
    <PharmacyLayout>
        ...
        <div class="pharmacy-content-shell flex min-h-full flex-1 flex-col">
                ...
                >
                    ...
                    >
                        <span
                            ...
                            :class="onglet === 'nouvelles' ? 'bg-white/25' : 'bg-[#3995d2]/15 text-[#3995d2]'"
                        >
                            {{ stats.nouvelles }}
                        </span>
                    ...
                    >
                        <span
                            class="inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded-full px-1 text-[10px] font-bold tabular-nums"
                            :class="onglet === 'en_attente' ? 'bg-white/25' : 'bg-indigo-100 text-indigo-700'"
                        >
                            {{ stats.en_attente }}
                        </span>
                    ...
                    >
                        <span
                            class="inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded-full px-1 text-[10px] font-bold tabular-nums"
                            :class="onglet === 'a_preparer' ? 'bg-white/25' : 'bg-amber-100 text-amber-800'"
                        >
                            {{ stats.a_preparer }}
                        </span>
                    ...
                    >
                        <span
                            class="inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded-full px-1 text-[10px] font-bold tabular-nums"
                            :class="onglet === 'livrees' ? 'bg-white/25' : 'bg-emerald-100 text-emerald-800'"
                        >
                            {{ stats.livrees }}
                        </span>
                        Retirées
                ...
                <template v-if="onglet === 'nouvelles'">
                    <div v-for="cmd in commandes.data" :key="cmd.id"
                         class="overflow-hidden rounded-2xl bg-white shadow-sm">
                        <!-- En-tête -->
                        <div class="flex cursor-pointer items-start justify-between px-5 py-4"
                             @click="toggleCard(cmd)">
                            <div class="flex-1 min-w-0">
                                <div class="mb-1.5 flex flex-wrap items-center gap-2">
                                    <span class="font-mono text-[15px] font-extrabold text-[#2563EB]">{{ cmd.numero }}</span>
                                    <span class="rounded-full bg-[#DBEAFE] px-2.5 py-0.5 text-[10px] font-bold text-[#1D4ED8]">Nouvelle commande 🔄</span>
                                ...
                                <div class="flex flex-wrap items-center gap-4 text-[12px] text-gray-500">
                                    <span class="flex items-center gap-1"><Paperclip class="size-3.5" />{{ cmd.produits?.length ?? 0 }} Médicaments demandés</span>
                                    <span class="flex items-center gap-1"><Clock class="size-3.5" />{{ cmd.date }}</span>
                                </div>
                                <p class="mt-1 flex items-center gap-1 text-[12px] font-medium"
                                   :class="cmd.ordonnance_id ? 'text-primary' : 'text-gray-400'">
                                    <FileText class="size-3.5" />
                                    <span v-if="cmd.ordonnance_id">Ordonnance à vérifier</span>
                                    <span v-else>Aucune ordonnance</span>
                                </p>
                            </div>
                            <component :is="expandedCards.has(cmd.id) ? ChevronUp : ChevronDown"
                                       class="mt-1 size-5 shrink-0 text-gray-400" />
                        </div>

                        <!-- Corps développé -->
                        <div v-if="expandedCards.has(cmd.id)"
                             class="border-t border-gray-100 px-5 pb-5 pt-4 space-y-4">
                            <!-- Box ordonnance -->
                            <div v-if="cmd.ordonnance_id"
                                 class="flex items-center justify-between rounded-xl border border-[#BFDBFE] bg-[#EFF6FF] px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="flex size-9 items-center justify-center rounded-full bg-[#3B82F6]/10">
                                        <FileText class="size-5 text-[#3B82F6]" />
                                    </div>
                                    <div>
                                        <p class="text-[13px] font-semibold text-gray-800">Ordonnance médicale jointe</p>
                                        <p class="text-[11px] text-[#F59E0B] font-medium">À vérifier</p>
                                    </div>
                                </div>
                                <button class="flex items-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-[11px] font-bold text-white shadow hover:bg-primary/90 transition-colors"
                            ...
                            <div>
                                <p class="mb-2.5 flex items-center gap-1.5 text-[12px] font-semibold text-gray-500">
                                ...
                                <div class="overflow-x-auto rounded-xl border border-gray-100">
                                    <table class="w-full min-w-[560px] text-[13px]">
                                        ...
                                        <tbody class="divide-y divide-gray-50">
                                            <tr v-for="p in cmd.produits" :key="p.id"
                                                class="transition-opacity"
                                                :class="formLignes[cmd.id]?.[p.id]?.dispo ? 'opacity-100' : 'opacity-40'">
                                                <!-- Nom -->
                                                <td class="px-4 py-2.5">
                                                    <span class="inline-block rounded-md border border-gray-200 bg-white px-2.5 py-1 text-[13px] text-gray-800">
                                                        {{ p.designation }}
                                                    </span>
                                                </td>
                                                ...
                                                <td class="px-3 py-2.5">
                                                    <div class="flex w-16 flex-col gap-0.5">
                                                        <input
                                                            v-model="formLignes[cmd.id][p.id].quantite"
                                                            type="number"
                                                            min="1"
                                                            :max="p.pivot.quantite"
                                                            :disabled="!formLignes[cmd.id]?.[p.id]?.dispo"
                                                            class="w-full rounded-md border px-2 py-1 text-center text-[13px] transition-colors"
                                                            :class="!formLignes[cmd.id]?.[p.id]?.dispo
                                                                ? 'border-gray-100 bg-gray-50 text-gray-400 cursor-not-allowed'
                                                                : qteInvalide(cmd.id, p)
                                                        ...
                                                        <!-- Hint max -->
                                                        <span
                                                            v-if="formLignes[cmd.id]?.[p.id]?.dispo"
                                                            class="w-full text-center text-[10px] leading-none"
                                                            :class="qteInvalide(cmd.id, p) ? 'text-red-500 font-semibold' : 'text-gray-400'"
                                                        >
                                                            max {{ p.pivot.quantite }}
                                                        </span>
                                                    </div>
                                                ...
                                                <td class="px-3 py-2.5">
                                                    <div class="flex items-center gap-1">
                                                        <input
                                                            v-model="formLignes[cmd.id][p.id].prix"
                                                            type="number"
                                                            min="0"
                                                            placeholder="Ex : 1000"
                                                            :disabled="!formLignes[cmd.id]?.[p.id]?.dispo"
                                                            ...
                                                            :class="formLignes[cmd.id]?.[p.id]?.dispo
                                                                ...
                                                                : 'border-gray-100 bg-gray-50 text-gray-400 cursor-not-allowed'"
                                                        />
                                                        <span class="text-[11px] text-gray-400 shrink-0">xaf</span>
                                                    </div>
                                                </td>
                                                <!-- Total calculé -->
                                                <td class="px-3 py-2.5">
                                                    <div class="flex items-center gap-1">
                                                        <span class="text-[13px] font-semibold"
                                                              :class="totalLigne(cmd.id, p) ? 'text-gray-900' : 'text-gray-300'">
                                                            {{ totalLigne(cmd.id, p) || 'Ex : 1000' }}
                                                        </span>
                                                        <span class="text-[11px] text-gray-400 shrink-0">xaf</span>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="px-3 py-2.5">
                                                    <button
                                                        ...
                                                        :class="formLignes[cmd.id][p.id].dispo ? 'bg-[#22C55E]' : 'bg-gray-200'"
                                                        @click="formLignes[cmd.id][p.id].dispo = !formLignes[cmd.id][p.id].dispo"
                                                    >
                                                        <span
                                                            class="inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform"
                                                            :class="formLignes[cmd.id][p.id].dispo ? 'translate-x-4' : 'translate-x-0.5'"
                                                        />
                                                    </button>
                                                </td>
                                            </tr>
                            ...
                            <div class="flex items-baseline gap-2 px-1">
                                <span class="text-[13px] font-bold text-gray-700">Total montant commande :</span>
                                <span class="text-2xl font-bold text-gray-900">{{ totalCmd(cmd).toFixed(1) }}</span>
                                <span class="text-[12px] text-gray-500">xaf</span>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <p v-if="hasQteError(cmd)"
                                   class="flex items-center gap-1.5 text-[12px] font-semibold text-red-500">
                                    <AlertCircle class="size-4 shrink-0" />
                                    La quantité confirmée ne peut pas dépasser la quantité demandée.
                                </p>
                                <div v-else class="flex-1" />
                                <div class="flex flex-col items-end gap-1">
                                    <p v-if="hasPrixError(cmd)" class="text-[11px] font-medium text-red-500">
                ...
                <template v-else-if="onglet === 'en_attente'">
                    <!-- Bandeau info violet -->
                    <div class="rounded-xl border border-[#C7D2FE] bg-[#EEF2FF] px-4 py-3">
                        <p class="flex items-center gap-2 text-[13px] font-bold text-[#3730A3]">
                            <AlertCircle class="size-4 shrink-0" />
                    ...
                    <div v-for="cmd in commandes.data" :key="cmd.id"
                        ...
                        <div class="flex cursor-pointer items-start justify-between px-5 py-4"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="mb-1.5 flex flex-wrap items-center gap-2">
                                    <span class="font-mono text-[15px] font-extrabold text-[#4338CA]">{{ cmd.numero }}</span>
                                    <span v-if="cmd.status_pharmacie === 'indisponible'"
                                ...
                                <div class="flex flex-wrap items-center gap-4 text-[12px] text-gray-500">
                                    <span class="flex items-center gap-1"><Paperclip class="size-3.5" />{{ cmd.produits?.length ?? 0 }} Médicaments</span>
                                    <span class="flex items-center gap-1"><Clock class="size-3.5" />{{ cmd.date }}</span>
                                </div>
                                <p class="mt-1 flex items-center gap-1 text-[12px] font-medium"
                                   :class="cmd.ordonnance_id ? 'text-[#22C55E]' : 'text-gray-400'">
                                    <FileText class="size-3.5" />
                                    <span v-if="cmd.ordonnance_id">Ordonnance soumise</span>
                                    <span v-else>Aucune ordonnance</span>
                                </p>
                            </div>
                            <component :is="expandedCards.has(cmd.id) ? ChevronUp : ChevronDown"
                        ...
                        <div v-if="expandedCards.has(cmd.id)"
                            ...
                            <div v-if="cmd.ordonnance_id"
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-[13px] font-semibold text-gray-800">Ordonnance médicale jointe</p>
                                        <p class="text-[11px] text-[#22C55E] font-medium">Soumise</p>
                                    </div>
                                </div>
                                <button class="flex items-center gap-1.5 rounded-lg border border-[#3B82F6] px-3 py-1.5 text-[11px] font-bold text-[#3B82F6] hover:bg-[#EFF6FF] transition-colors"
                                        @click.stop="openOrdonnance(cmd)">
                                ...
                                <div class="overflow-x-auto rounded-xl border border-gray-100">
                                    <table class="w-full min-w-[560px] text-[13px]">
                                        ...
                                        <tbody class="divide-y divide-gray-50">
                                            <tr v-for="p in cmd.produits" :key="p.id">
                                                <td class="px-4 py-2.5">
                                                    <span class="inline-block rounded-md border border-gray-200 bg-white px-2.5 py-1 text-[13px]"
                                                          :class="p.pivot.status === 'indisponible' ? 'text-red-400 line-through' : 'text-gray-800'">
                                                        {{ p.designation }}
                                                    </span>
                                                </td>
                                                ...
                                                <td class="px-3 py-2.5">
                                                    <span class="relative inline-flex h-5 w-9 items-center rounded-full"
                                                          :class="p.pivot.status !== 'indisponible' ? 'bg-[#22C55E]' : 'bg-gray-200'">
                                                        <span class="inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform"
                                                              :class="p.pivot.status !== 'indisponible' ? 'translate-x-4' : 'translate-x-0.5'" />
                                                    </span>
                                                </td>
                                            </tr>
                                        </tbody>
                ...
                <template v-else-if="onglet === 'a_preparer'">
                    <div class="rounded-xl border border-[#FCD34D] bg-[#FFFBEB] px-4 py-3">
                        <p class="text-[13px] font-bold text-[#92400E]">Commandes validées par les clients</p>
                        <p class="mt-0.5 text-[12px] text-[#78350F]">
                            Ces commandes ont été confirmées par les patients. Veuillez les préparer pour la livraison ou le retrait.
                        </p>
                    </div>
                    ...
                    <div v-for="cmd in commandes.data" :key="cmd.id"
                        ...
                        <div class="flex items-start justify-between px-5 py-4">
                            <div class="flex-1 min-w-0 cursor-pointer" @click="toggleCard(cmd)">
                                <div class="mb-1.5 flex flex-wrap items-center gap-2">
                                    <span class="font-mono text-[15px] font-extrabold text-[#2563EB]">{{ cmd.numero }}</span>
                                    <span class="rounded-full bg-[#FEF3C7] px-2.5 py-0.5 text-[10px] font-bold text-[#92400E]">Commande Validée À préparer 🔄</span>
                                    <span v-if="cmd.ordonnance_id"
                                          class="flex items-center gap-1 rounded-full bg-[#E0F2FE] px-2.5 py-0.5 text-[10px] font-bold text-[#0369A1]">
                                        <FileText class="size-3" />Ordonnance
                                    </span>
                                ...
                                <p class="mt-1 flex items-center gap-1 text-[12px] font-medium"
                                   :class="cmd.ordonnance_id ? 'text-[#22C55E]' : 'text-gray-400'">
                                    ...
                                    <span v-if="cmd.ordonnance_id">Ordonnance vérifiée</span>
                                    <span v-else>Aucune ordonnance</span>
                                </p>
                            </div>
                        ...
                        <div v-if="expandedCards.has(cmd.id)"
                            ...
                            <div v-if="cmd.ordonnance_id"
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-[13px] font-semibold text-gray-800">Ordonnance médicale jointe</p>
                                        <p class="text-[11px] text-[#22C55E] font-medium">Vérifier</p>
                                    </div>
                                </div>
                                <button class="flex items-center gap-1.5 rounded-lg border border-[#3B82F6] px-3 py-1.5 text-[11px] font-bold text-[#3B82F6] hover:bg-[#EFF6FF] transition-colors"
                                        @click.stop="openOrdonnance(cmd)">
                            ...
                            <div class="flex items-baseline gap-2 px-1">
                                <span class="text-[13px] font-bold text-gray-700">Total montant commande :</span>
                                <span class="text-2xl font-bold text-gray-900">{{ Number(cmd.prix_total || 0).toFixed(1) }}</span>
                                <span class="text-[12px] text-gray-500">xaf</span>
                            </div>

                            <!-- Commentaire (lecture seule) -->
                            <div class="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-[13px]"
                                 :class="cmd.commentaire ? 'text-gray-700' : 'text-gray-400'">
                ...
                <template v-else-if="onglet === 'livrees'">
                    <div v-for="cmd in commandes.data" :key="cmd.id"
                        ...
                        <div class="flex cursor-pointer items-start justify-between px-5 py-4"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="mb-1.5 flex flex-wrap items-center gap-2">
                                    <span class="font-mono text-[15px] font-extrabold text-gray-700">{{ cmd.numero }}</span>
                                    <span class="rounded-full bg-[#DCFCE7] px-2.5 py-0.5 text-[10px] font-bold text-[#15803D]">Retirée ✓</span>
                                </div>
                                <div class="flex flex-wrap items-center gap-4 text-[12px] text-gray-500">
                                    <span class="flex items-center gap-1"><Paperclip class="size-3.5" />{{ cmd.produits?.length ?? 0 }} Médicaments</span>
                                    <span class="flex items-center gap-1"><CheckCircle2 class="size-3.5 text-[#22C55E]" />{{ cmd.date }}</span>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="rounded-lg bg-[#DCFCE7] px-3 py-1 text-[11px] font-bold text-[#15803D]">Retirée</span>
                                <component :is="expandedCards.has(cmd.id) ? ChevronUp : ChevronDown"
                                           class="size-5 text-gray-400" />
                            </div>
                        </div>
            ...
            >
                <p class="text-center text-[13px] leading-snug text-[#5c5959] sm:text-left sm:text-[14px]">
                    Affichage de
                    <span class="tabular-nums font-black text-black">{{ commandes.from }}</span>
                    à
                    <span class="tabular-nums font-black text-black">{{ commandes.to }}</span>
                    sur
                    <span class="tabular-nums font-black text-[#3995d2]">{{ commandes.total }}</span>
                    commandes
                </p>
                <nav class="flex flex-wrap items-center justify-center gap-1.5 sm:justify-end" aria-label="Pagination">
        ...
        <Teleport to="body">
            <div v-if="confirmModal.open"
                ...
                <div class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
                    <!-- Entête -->
                    <div class="flex items-center gap-3 bg-[#FFFBEB] border-b border-[#FCD34D] px-6 py-4">
                        <div class="flex size-10 shrink-0 items-center justify-center rounded-full bg-[#F59E0B]/10">
                            <ShoppingCart class="size-5 text-[#F59E0B]" />
                        </div>
                        <div>
                            <p class="text-[15px] font-extrabold text-gray-900">Confirmer la récupération</p>
                            <p class="text-[12px] text-gray-500">Commande {{ confirmModal.cmd?.numero }}</p>
                        </div>
                    </div>
                    <!-- Corps -->
                    <div class="px-6 py-5">
                        <p class="text-[14px] text-gray-700">
                            Vous êtes sur le point de valider que le patient a bien récupéré ou payé sa commande.
                        </p>
                        <p class="mt-2 text-[13px] font-semibold text-[#92400E] bg-[#FFFBEB] rounded-lg px-3 py-2 border border-[#FCD34D]">
                            ⚠ Cette action est irréversible. La commande passera en statut « Retirée ».
                        </p>
                    </div>
                    <!-- Pied -->
        ...
        <Teleport to="body">
            <div v-if="ordModal.open"
                 class="fixed inset-0 z-50 flex items-center justify-center p-4"
                 style="background: rgba(0,0,0,0.55)"
                 @click.self="closeOrdonnance">
                <div class="relative flex w-full max-w-[500px] max-h-[90vh] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
                    <!-- Entête -->
                    <div class="flex items-center gap-3 border-b border-gray-100 px-5 py-4">
                        <div class="flex size-9 shrink-0 items-center justify-center rounded-full bg-[#EFF6FF]">
                            <FileText class="size-5 text-[#3B82F6]" />
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-[14px] font-extrabold text-gray-900">Ordonnance — Commande {{ ordModal.numero }}</p>
                            <p class="text-[11px] text-gray-400">Vérification d'authenticité</p>
                        </div>
                        <button class="flex size-8 shrink-0 items-center justify-center rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 transition-colors"
                    ...
                    <div class="flex items-center gap-2 border-b border-gray-100 bg-gray-50 px-4 py-2">
                        <button class="flex size-7 items-center justify-center rounded-md border border-gray-200 bg-white text-gray-600 hover:bg-gray-100"
                                @click="zoomOut"><ZoomOut class="size-3.5" /></button>
                        <span class="min-w-[40px] text-center text-[12px] font-semibold text-gray-700">{{ zoom }}%</span>
                        <button class="flex size-7 items-center justify-center rounded-md border border-gray-200 bg-white text-gray-600 hover:bg-gray-100"
                                @click="zoomIn"><ZoomIn class="size-3.5" /></button>
                        <button class="flex size-7 items-center justify-center rounded-md border border-gray-200 bg-white text-gray-600 hover:bg-gray-100"
                                @click="resetZoom"><RefreshCw class="size-3.5" /></button>
                        <div class="flex-1" />
                        <button class="flex items-center gap-1.5 rounded-md border border-gray-200 bg-white px-2.5 py-1.5 text-[11px] font-semibold text-gray-600 hover:bg-gray-100"
                                @click="downloadOrdonnance">
                            <Download class="size-3.5" />Télécharger
                        </button>
                    </div>
                    <!-- Visionneuse -->
                    <div class="flex-1 overflow-auto bg-gray-100 flex items-start justify-center min-h-[250px] p-4">
                        <img v-if="ordModal.url" :src="ordModal.url" alt="Ordonnance"
                    ...
                    <div class="border-t border-gray-100 px-5 py-4 space-y-3">
                        <div class="rounded-xl bg-[#EFF6FF] px-4 py-3">
                            <div class="mb-1 flex items-center gap-2">
                                ...
                                <span class="text-[13px] font-bold text-[#1D4ED8]">Prêt à vérifier</span>
                            </div>
                            <p class="mb-3 text-[11px] text-[#3B82F6]">Analyse par IA</p>
                            <button class="w-full rounded-xl bg-[#3B82F6] py-2.5 text-[13px] font-bold text-white shadow hover:bg-[#2563EB] transition-colors">
                                Lancer l'authentification
                            </button>
                        </div>
                        <div>
                            <p class="mb-2 flex items-center gap-1.5 text-[12px] font-bold text-[#3B82F6]">
                                <Eye class="size-3.5" />Point de vérification
                            </p>
                            <ul class="space-y-1 text-[12px] text-gray-600">
                                <li class="flex items-center gap-2"><span class="size-1.5 rounded-full bg-gray-400 shrink-0" />Cachet du médecin</li>
                                <li class="flex items-center gap-2"><span class="size-1.5 rounded-full bg-gray-400 shrink-0" />Signature du médecin</li>
                                <li class="flex items-center gap-2"><span class="size-1.5 rounded-full bg-gray-400 shrink-0" />Date de prescription</li>
                                <li class="flex items-center gap-2"><span class="size-1.5 rounded-full bg-gray-400 shrink-0" />Nom du patient</li>
                            </ul>
                        </div>
                    </div>
                </div>
