...
const revenuScaleMax = computed(() => Math.max(5000, Math.ceil(maxRevenuRaw / 5000) * 5000));
...
<template>
    ...
    <PharmacyLayout>
        <div class="pharmacy-content-shell">
                ...
                <div class="relative z-[1] mx-auto mb-6 w-full max-w-[1346px] pb-28 lg:pb-36">
                    <div class="pharmacy-card pharmacy-card--hero pharmacy-hero-figma">
                        <p class="pharmacy-hero-figma__title px-6 pb-3 pt-6 lg:pb-0 lg:pt-0 lg:px-0">
                        ...
                        <div class="pharmacy-hero-figma__phone hidden lg:block" aria-hidden="true">
                            <img
                                src="/images/pharmacy/dashboard-hero-phone-hand.png"
                                alt=""
                                width="317"
                ...
                <div class="relative z-[2] mb-6 grid gap-4 sm:grid-cols-3">
                    <div class="pharmacy-card p-5 sm:p-6">
                        <div class="mb-2 flex items-start justify-between">
                            <p class="text-sm font-black text-black sm:text-base">Revenu total</p>
                            <button type="button" class="rounded-lg p-1 text-black/40 hover:bg-black/5" aria-label="Options">
                                <MoreVertical class="size-5" />
                            </button>
                        </div>
                        <p class="text-[clamp(1.75rem,4vw,2.8rem)] font-black tabular-nums leading-none text-black">
                            {{ stats.revenu_total?.toLocaleString('fr-FR') ?? 0 }}
                            <span class="text-lg font-black text-black/80 sm:text-xl">XAF</span>
                        </p>
                        <div class="mt-4 flex flex-wrap items-center gap-3">
                            <span
                                class="inline-flex items-center gap-1 rounded-[13px] border border-black px-2.5 py-1 text-xs font-black text-black"
                            >
                                <TrendingUp v-if="(stats.pct_revenu ?? 0) >= 0" class="size-3.5 shrink-0" />
                    ...
                    <div class="pharmacy-card p-5 sm:p-6">
                        <div class="mb-2 flex items-start justify-between">
                            <p class="text-sm font-black text-black sm:text-base">Commandes totales</p>
                            <button type="button" class="rounded-lg p-1 text-black/40 hover:bg-black/5" aria-label="Options">
                                <MoreVertical class="size-5" />
                            </button>
                        </div>
                        <p class="text-[clamp(1.75rem,4vw,2.8rem)] font-black tabular-nums leading-none text-black">
                            ...
                            <span class="text-lg font-black text-black/80 sm:text-xl">cmd</span>
                        </p>
                        <div class="mt-4 flex flex-wrap items-center gap-3">
                            <span
                                class="inline-flex items-center gap-1 rounded-[13px] border border-black px-2.5 py-1 text-xs font-black text-black"
                            >
                                <TrendingUp v-if="(stats.pct_commandes ?? 0) >= 0" class="size-3.5 shrink-0" />
                                <TrendingDown v-else class="size-3.5 shrink-0" />
                    ...
                    <div class="pharmacy-card p-5 sm:p-6">
                        <div class="mb-2 flex items-start justify-between">
                            <p class="text-sm font-black text-black sm:text-base">Clients</p>
                            <button type="button" class="rounded-lg p-1 text-black/40 hover:bg-black/5" aria-label="Options">
                                <MoreVertical class="size-5" />
                            </button>
                        </div>
                        <p class="text-[clamp(1.75rem,4vw,2.8rem)] font-black tabular-nums leading-none text-black">
                            {{ stats.nb_clients ?? 0 }}
                            <span class="text-lg font-black text-black/80 sm:text-xl">clients</span>
                        </p>
                        <div class="mt-4 flex flex-wrap items-center gap-3">
                            <span
                                class="inline-flex items-center gap-1 rounded-[13px] border border-black px-2.5 py-1 text-xs font-black text-black"
                            >
                                <TrendingUp v-if="(stats.pct_clients ?? 0) >= 0" class="size-3.5 shrink-0" />
                                <TrendingDown v-else class="size-3.5 shrink-0" />
                ...
                <div class="pharmacy-card mb-6 p-5 sm:p-6">
                    ...
                    <div class="flex gap-2 sm:gap-3">
                        <div class="flex h-[220px] w-8 shrink-0 flex-col justify-between py-1 text-right text-xs font-medium text-black sm:w-9 sm:text-sm">
                            <span v-for="tick in revenuYTicks" :key="tick">{{ formatAxisK(tick) }}</span>
                        </div>
                        <div class="flex min-h-[220px] min-w-0 flex-1 items-end justify-between gap-1 overflow-x-auto pb-6 sm:gap-2">
                            <template v-for="(point, i) in revenusParJour" :key="i">
                                <div class="flex min-w-[36px] flex-1 flex-col items-center gap-2 sm:min-w-[44px]">
                                    <div
                                    ...
                                    <span class="text-xs font-medium text-black">{{ point.label }}</span>
                                </div>
                            </template>
                ...
                <div class="grid gap-6 lg:grid-cols-2">
                    <div class="pharmacy-card p-5 sm:p-6">
                        ...
                        <div class="flex min-h-[160px] items-end justify-between gap-1 overflow-x-auto pb-2 sm:gap-2">
                            <template v-for="(point, i) in volumeParJour" :key="i">
                                <div class="flex min-w-[32px] flex-1 flex-col items-center gap-2 sm:min-w-[40px]">
                                    <div
                                    ...
                                    <span class="text-[11px] font-medium text-black sm:text-xs">{{ point.label }}</span>
                                </div>
                            </template>
                    ...
                    <div class="pharmacy-card p-5 sm:p-6">
                        ...
                        <div class="flex min-h-[200px] items-end justify-around gap-3 pb-2">
                            <template v-for="(item, i) in meilleursVentes.slice(0, 3)" :key="item.id">
                                <div class="flex min-w-0 max-w-[33%] flex-1 flex-col items-center gap-2">
                                    <div
                                    ...
                                    >
                                        <div
                                            ...
                                            :style="{
                                                ...
                                                minHeight: item.ca > 0 ? '12px' : '0',
                                                backgroundColor: barColors[i % barColors.length],
                                            }"
                                        />
                                    </div>
                                    <p class="line-clamp-2 max-w-full text-center text-[11px] font-medium text-black sm:text-xs">
                                        {{ item.designation }}
                                    </p>
                                    <p class="text-[11px] font-black text-black sm:text-xs">
