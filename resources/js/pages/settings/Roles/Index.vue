...
const activeTab = ref<'backoffice' | 'pharmacie'>(props.filters.onglet === 'pharmacie' ? 'pharmacie' : 'backoffice');
...
const rolesDisplay = computed(() =>
    activeTab.value === 'backoffice' ? props.rolesBackoffice : props.rolesPharmacie
);
...
function ouvrirCreate() {
    formCreate.value = { name: '', description: '', type: activeTab.value, permissions: [] };
...
function permissionsApercu(role: RoleItem): Array<{ name: string; label: string }> {
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <SettingsLayout>
            <div class="space-y-6">
                ...
                <div class="flex gap-2">
                    <button
                        ...
                        :class="activeTab === 'backoffice' ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'"
                        @click="activeTab = 'backoffice'; filtrer()"
                    >
                    ...
                    <button
                        type="button"
                        class="rounded-lg px-4 py-2 text-sm font-medium transition-colors"
                        :class="activeTab === 'pharmacie' ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'"
                        @click="activeTab = 'pharmacie'; filtrer()"
                    >
                    ...
                    >
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <div class="flex shrink-0 gap-1">
                                ...
                                <Button variant="ghost" size="icon" class="size-8" title="Modifier" @click="ouvrirEdit(role)">
                                    <Pencil class="size-4" />
                                </Button>
                                <Button
                        ...
                        <div class="mt-4 min-w-0 space-y-1.5 overflow-hidden">
                            <template v-for="(perm, i) in permissionsApercu(role)" :key="perm.name">
                                <div
                            ...
                            <div
                                v-if="role.permissions.length > permissionsApercu(role).length"
                                class="rounded-lg bg-muted/60 px-2.5 py-1.5 text-center text-xs text-muted-foreground"
                            >
                                + {{ role.permissions.length - permissionsApercu(role).length }} {{ role.permissions.length - permissionsApercu(role).length === 1 ? 'autre' : 'autres' }}
                            </div>
                        </div>
                    </div>
                </div>

                <p v-if="!filteredRoles.length" class="py-12 text-center text-muted-foreground">
                    Aucun rôle trouvé.
                </p>
            </div>
            ...
            <Dialog :open="modalCreate" @update:open="modalCreate = $event">
                <DialogContent class="max-h-[90vh] max-w-lg overflow-y-auto">
                    ...
                    <form class="space-y-4" @submit.prevent="creerRole">
                        <div class="space-y-2">
                            <Label>Libellé</Label>
                            <Input v-model="formCreate.name" placeholder="Ex : agent call-center" required />
                        </div>
                        <div class="space-y-2">
                            <Label>Description</Label>
                            <Input v-model="formCreate.description" placeholder="Responsabilités du rôle" />
                        </div>
                        <div class="space-y-2">
                            <Label>Type</Label>
                            <select
                            ...
                            >
                                <option value="backoffice">Rôles Utilisateurs Backoffice</option>
                                <option value="pharmacie">Rôles agents pharmacie</option>
                            </select>
                        </div>
                        <div class="space-y-3">
                            <Label>Permissions</Label>
                            <div v-for="(perms, cat) in permissionsGroupées" :key="cat" class="space-y-2">
                                <p class="text-sm font-medium text-muted-foreground">{{ cat }}</p>
                                <div class="flex flex-wrap gap-2">
                                    <label
                                        v-for="p in perms"
                                        :key="p.name"
                                        class="inline-flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm transition-colors"
                                        :class="formCreate.permissions.includes(p.name) ? 'border-primary bg-primary/5' : 'hover:bg-muted/50'"
                                    >
                                        <input
                                            type="checkbox"
                                            :checked="formCreate.permissions.includes(p.name)"
                                            class="rounded"
                                            @change="toggleCreatePermission(p.name)"
                                        />
                                        {{ p.label }}
                                    </label>
                                </div>
                        ...
                        <DialogFooter>
                            <Button type="button" variant="outline" @click="modalCreate = false">Annuler</Button>
            ...
            <Dialog :open="modalDetail" @update:open="modalDetail = $event">
                <DialogContent class="max-w-lg">
                    ...
                    <div v-if="roleDetail" class="space-y-4">
                        <p class="text-sm text-muted-foreground">{{ roleDetail.description }}</p>
                        <div class="flex gap-4">
                            <div class="flex items-center gap-2">
                                <Users class="size-4" />
                                <span class="text-sm font-medium">{{ roleDetail.users_count }} utilisateur(s)</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <ShieldCheck class="size-4" />
                                <span class="text-sm font-medium">{{ roleDetail.permissions_count }} permission(s)</span>
                            </div>
                        </div>
                        <div class="space-y-2">
                            <Label>Permissions</Label>
                        ...
                        <DialogFooter>
                            ...
                            <Button @click="ouvrirEdit(roleDetail); modalDetail = false">Modifier</Button>
                        </DialogFooter>
                    </div>
                </DialogContent>
            </Dialog>
            ...
            <!-- Modal Modifier rôle -->
            <Dialog :open="modalEdit" @update:open="(v) => { if (!v) { modalEdit = false; roleToEdit = null; } }">
                <DialogContent class="max-h-[90vh] max-w-lg overflow-y-auto">
                    ...
                    <form v-if="roleToEdit" class="space-y-4" @submit.prevent="modifierRole">
                        <div class="space-y-3">
                            <Label>Permissions</Label>
                            <div v-for="(perms, cat) in permissionsGroupées" :key="cat" class="space-y-2">
                                <p class="text-sm font-medium text-muted-foreground">{{ cat }}</p>
                                <div class="flex flex-wrap gap-2">
                                    <label
                                        ...
                                        :key="p.name"
                                        class="inline-flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm transition-colors"
                                        :class="formEdit.permissions.includes(p.name) ? 'border-primary bg-primary/5' : 'hover:bg-muted/50'"
                                    >
                                        <input
                                            type="checkbox"
                                            :checked="formEdit.permissions.includes(p.name)"
                                            class="rounded"
                                            @change="toggleEditPermission(p.name)"
                                        />
                                        {{ p.label }}
                                    </label>
                                </div>
                        ...
                        <DialogFooter>
                            <Button type="button" variant="outline" @click="modalEdit = false">Annuler</Button>
            ...
            <ConfirmModal
                ...
                :description="roleToDelete ? `Supprimer le rôle « ${roleToDelete.label} » ? Cette action est irréversible.` : ''"
