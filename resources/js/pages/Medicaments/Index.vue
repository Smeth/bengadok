...
import { Search, Link2, Package, LayoutGrid, List, Database, Pencil, Trash2, Plus } from 'lucide-vue-next';
...
const activeTab = ref<TabId>(props.onglet === 'db_medicament' ? 'db_medicament' : 'catalogue');
...
function goCatalogueTab() {
    ...
    if (wasDb) {
        router.get(
            ...
            { preserveState: true, preserveScroll: true, only: ['produits', 'onglet', 'filters'] },
        );
    }
}
...
function goDbMedicamentTab() {
    ...
    router.get(
        ...
        { preserveState: true, preserveScroll: true, only: ['dbMedicaments', 'onglet'] },
    );
}
...
function openEditMedicament(row: DbMedicamentRow) {
    ...
    medForm.value = {
        ...
        prix: row.prix != null && Number.isFinite(row.prix) ? String(row.prix) : '',
        notes: row.notes ?? '',
    };
    medModalOpen.value = true;
}
...
function submitMedModal() {
    ...
    if (medModalMode.value === 'create') {
        ...
        router.patch(`/medicaments/db-medicaments/${editingId.value}`, payload, {
            preserveScroll: true,
            onSuccess: () => closeMedModal(),
        });
    }
...
function destroyRow(id: number) {
    ...
    router.delete(`/medicaments/db-medicaments/${id}`, { preserveScroll: true });
}

const badgeTotal = computed(() =>
    activeTab.value === 'db_medicament' ? props.dbMedicaments.total : props.produits.total,
);
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            ...
            <!-- Badge + tabs -->
            <div class="relative z-10 flex shrink-0 flex-wrap items-center justify-between gap-4">
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <button
                            ...
                            :class="activeTab === 'db_medicament' ? 'bg-[#459cd1] text-white' : 'bg-white/80 text-muted-foreground hover:bg-white'"
            ...
            <div v-if="activeTab === 'catalogue'" class="space-y-4">
                <form class="flex flex-wrap gap-4" @submit.prevent="filtrer('search', searchQuery)">
                    <div class="relative min-w-[200px] flex-1">
                        ...
                        <Input v-model="searchQuery" placeholder="Rechercher un médicament..." class="pl-9" />
                    </div>
                    <select
                        :value="filters.tri"
                    ...
                    >
                        ...
                        <div class="space-y-2 text-sm">
                            <p class="flex flex-wrap gap-1">
                                <span class="text-muted-foreground">Forme:</span><span class="ml-1 font-medium">{{ p.forme || '-' }}</span>
                            </p>
                            <p class="flex flex-wrap gap-1">
                                <span class="text-muted-foreground">Prix moyen:</span
                                ><strong class="ml-1 text-emerald-600">{{ Number(p.prix_moyen).toLocaleString('fr-FR') }} xaf</strong>
                            </p>
                            <p class="flex flex-wrap gap-1">
                                <span class="text-muted-foreground">Fourchette:</span
                                ><strong class="ml-1 text-emerald-600"
                                    >{{ Number(p.prix_min).toLocaleString('fr-FR') }} - {{ Number(p.prix_max).toLocaleString('fr-FR') }} xaf</strong
                                >
                            </p>
                            <p class="flex flex-wrap gap-1">
                                <span class="text-muted-foreground">Vente:</span
                                ><strong class="ml-1 text-emerald-600">{{ p.ventes }} Unités</strong>
                            </p>
                            <p class="flex flex-wrap gap-1">
                                <span class="text-muted-foreground">CA généré:</span
                                ><strong class="ml-1 text-emerald-600">{{ Number(p.ca).toLocaleString('fr-FR') }} xaf</strong>
                            </p>
                        </div>
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3 text-right font-medium text-emerald-600">
                                        {{ Number(p.prix_moyen).toLocaleString('fr-FR') }} xaf
                                    </td>
                                    <td class="px-4 py-3 text-right text-muted-foreground">
                                        {{ Number(p.prix_min).toLocaleString('fr-FR') }} - {{ Number(p.prix_max).toLocaleString('fr-FR') }} xaf
                                    </td>
                                    <td class="px-4 py-3 text-right">{{ p.ventes }} Unités</td>
                                    <td class="px-4 py-3 text-right font-medium text-emerald-600">
                                        {{ Number(p.ca).toLocaleString('fr-FR') }} xaf
                                    </td>
                    ...
                    <div class="flex flex-wrap items-center gap-1">
                        <template v-for="(link, pIndex) in produits.links" :key="pIndex">
                            ...
                            <Link
                                ...
                                :class="link.active ? 'border-[#459cd1] bg-[#459cd1] text-white' : 'border-input bg-white text-foreground'"
                                v-html="link.label"
                            />
                        </template>
                    </div>
            ...
            >
                <div class="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-start sm:justify-between">
                    <div class="min-w-0 space-y-1">
                        <div class="flex items-center gap-2">
                            <div class="flex size-10 items-center justify-center rounded-xl bg-[#459cd1]/15 text-[#459cd1]">
                                <Database class="size-5" />
                            </div>
                            <h2 class="text-xl font-semibold tracking-tight text-foreground">DB médicament</h2>
                        </div>
                        <p class="max-w-2xl text-sm leading-relaxed text-muted-foreground">
                            Référentiel local indépendant du catalogue et des commandes. Utilisez le bouton ci-contre pour saisir une fiche complète (prix, laboratoire, etc.).
                        </p>
                        <p class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-800/50 dark:bg-amber-900/20 dark:text-amber-200">
                            <strong>Module isolé :</strong> Les médicaments ajoutés ici ne sont pas utilisés dans les commandes, le catalogue ou la recherche produit. Ce module est indépendant du reste du système.
                        </p>
                    </div>
                    <Button
                        v-if="isAdmin"
                ...
                <p v-if="!isAdmin" class="text-sm text-muted-foreground">
                    Seuls les administrateurs peuvent ajouter ou modifier des entrées.
                </p>
                ...
                <div class="overflow-x-auto rounded-xl border border-border">
                    <table class="w-full min-w-[900px] text-sm">
                        <thead class="border-b bg-muted/50">
                            <tr>
                                <th class="px-3 py-3 text-left font-medium">Désignation</th>
                                <th class="px-3 py-3 text-left font-medium">Dosage</th>
                                <th class="px-3 py-3 text-left font-medium">Forme</th>
                                <th class="px-3 py-3 text-right font-medium">Prix (FCFA)</th>
                                <th class="px-3 py-3 text-left font-medium">Type</th>
                                <th class="px-3 py-3 text-left font-medium">Laboratoire</th>
                                <th class="px-3 py-3 text-left font-medium">Code</th>
                                <th class="px-3 py-3 text-left font-medium text-muted-foreground">Créé le</th>
                                <th v-if="isAdmin" class="px-3 py-3 text-right font-medium w-[100px]"></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr
                            ...
                            >
                                <td class="px-3 py-3 font-medium">{{ row.designation }}</td>
                                <td class="px-3 py-3 text-muted-foreground">{{ row.dosage || '—' }}</td>
                                <td class="px-3 py-3 text-muted-foreground">{{ row.forme || '—' }}</td>
                                <td class="px-3 py-3 text-right font-medium tabular-nums text-emerald-700">
                                    {{ row.prix != null ? Number(row.prix).toLocaleString('fr-FR') : '—' }}
                                </td>
                                <td class="px-3 py-3 text-muted-foreground">{{ row.type || '—' }}</td>
                                <td class="max-w-[140px] truncate px-3 py-3 text-muted-foreground" :title="row.laboratoire || ''">
                                    {{ row.laboratoire || '—' }}
                                </td>
                                <td class="px-3 py-3 font-mono text-xs text-muted-foreground">{{ row.code_article || '—' }}</td>
                                <td class="px-3 py-3 text-xs text-muted-foreground whitespace-nowrap">{{ row.created_at || '—' }}</td>
                                <td v-if="isAdmin" class="px-3 py-3 text-right">
                                    <div class="flex justify-end gap-0.5">
                                        <Button variant="ghost" size="icon" class="size-8" title="Modifier" @click="openEditMedicament(row)">
                                            <Pencil class="size-4" />
                                        </Button>
                                        <Button
                                            variant="ghost"
                ...
                <p v-if="!dbMedicaments.data.length" class="rounded-lg border border-dashed py-12 text-center text-sm text-muted-foreground">
                    Aucune entrée. Cliquez sur « Nouveau médicament » pour créer la première fiche.
                </p>

                <div v-if="dbMedicaments.links.length > 3" class="flex flex-wrap items-center justify-between gap-2 pt-1">
                    <p class="text-sm text-muted-foreground">
                        {{ dbMedicaments.from }}–{{ dbMedicaments.to }} sur {{ dbMedicaments.total }}
                    </p>
                    <div class="flex flex-wrap gap-1">
                        <template v-for="(link, i) in dbMedicaments.links" :key="i">
                            <div
                                v-if="link.url === null"
                            ...
                            <Link
                                ...
                                :class="link.active ? 'border-[#459cd1] bg-[#459cd1] text-white' : 'border-input bg-white'"
                                v-html="link.label"
                            />
                        </template>
                    </div>
                ...
                <Dialog :open="medModalOpen" @update:open="(v: boolean) => !v && closeMedModal()">
                    <DialogContent class="flex max-h-[min(90vh,720px)] max-w-2xl flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
                        <DialogHeader class="shrink-0 border-b px-6 py-4 text-left">
                            <DialogTitle class="text-lg">
                                {{ medModalMode === 'create' ? 'Nouveau médicament' : 'Modifier le médicament' }}
                            </DialogTitle>
                            <p class="mt-1 text-sm text-muted-foreground">
                                Fiche détaillée pour la base locale (hors flux commandes).
                            </p>
                        </DialogHeader>
                        <div class="min-h-0 flex-1 overflow-y-auto px-6 py-4">
                            <div class="grid gap-4 sm:grid-cols-2">
                                <div class="sm:col-span-2">
                                    <Label class="text-xs font-medium">Désignation <span class="text-destructive">*</span></Label>
                                    <Input
                                ...
                                <div>
                                    ...
                                    <Input v-model="medForm.dosage" class="mt-1.5" placeholder="Ex. 500 mg" />
                                </div>
                                <div>
                                    <Label class="text-xs font-medium">Forme galénique</Label>
                                    <Input v-model="medForm.forme" class="mt-1.5" placeholder="Comprimé, sirop, gélule…" />
                                </div>
                                <div>
                                    <Label class="text-xs font-medium">Prix unitaire (FCFA) <span class="text-destructive">*</span></Label>
                                    <Input
                                        ...
                                        placeholder="Ex. 2500"
                                    />
                                    <p v-if="medForm.prix.trim() && !medFormPrixValide" class="mt-1 text-xs text-destructive">
                                        Saisissez un montant valide (≥ 0).
                                    </p>
                                </div>
                                <div>
                                    <Label class="text-xs font-medium">Type de délivrance</Label>
                                    <select
                                    ...
                                    >
                                        <option v-for="opt in TYPE_DB_OPTIONS" :key="opt" :value="opt">{{ opt }}</option>
                                    </select>
                                </div>
                                <div>
                                    <Label class="text-xs font-medium">Laboratoire / fabricant</Label>
                                    <Input v-model="medForm.laboratoire" class="mt-1.5" placeholder="Optionnel" />
                                </div>
                                <div>
                                    <Label class="text-xs font-medium">Code article / référence</Label>
                                    <Input v-model="medForm.code_article" class="mt-1.5" placeholder="Optionnel" />
                                </div>
                                <div class="sm:col-span-2">
                        ...
                        <DialogFooter class="shrink-0 gap-2 border-t bg-muted/30 px-6 py-4 sm:justify-end">
                            <Button type="button" variant="outline" @click="closeMedModal">Annuler</Button>
                            <Button
                                type="button"
                            ...
                            >
                                {{ medModalMode === 'create' ? 'Créer' : 'Enregistrer' }}
                            </Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
