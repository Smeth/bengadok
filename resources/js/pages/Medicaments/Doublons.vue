...
const props = withDefaults(
    ...
    {
        groupes: () => [],
        stats: () => ({ en_attente: 0, verifies: 0, fusionnes: 0, total_produits: 0 }),
        filters: () => ({}),
        criteresDisponibles: () => ({
...
function ouvrirModalFusion(g: Groupe) {
    ...
    const suggere = g.medicaments.find((m) => m.is_principal) ?? g.medicaments[0];
...
function confirmerFusion() {
    ...
    if (!g || !pid) return;
    router.patch(`/medicaments/doublons/${g.id}/fusionner`, { principal_id: pid }, { preserveScroll: true });
    fermerModalFusion();
}
...
function confirmIgnorer() {
    ...
    router.patch(`/medicaments/doublons/${groupeToIgnorer.value.id}/ignorer`, {}, {
        preserveScroll: true,
...
function verifier(g: Groupe) {
    router.patch(`/medicaments/doublons/${g.id}/verifier`, {}, { preserveScroll: true });
}
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            ...
            <div v-if="flashError" class="rounded-lg bg-red-100 py-3 px-4 text-sm font-medium text-red-800 dark:bg-red-900/50">
                {{ flashError }}
            </div>
            ...
            <p class="rounded-lg border border-sky-200 bg-sky-50 px-4 py-2 text-xs text-sky-800 dark:border-sky-800/50 dark:bg-sky-900/20 dark:text-sky-200">
                <strong>Basé sur les commandes :</strong> Les doublons sont détectés parmi les médicaments (produits) présents dans les commandes, comme pour les clients.
            </p>

            <!-- Critères de détection -->
            <div class="rounded-xl border border-white/80 bg-white p-4 dark:border-white/10 dark:bg-white/95">
                <p class="mb-2 text-sm font-medium text-foreground">Critères de détection des doublons</p>
                <p class="mb-3 text-xs text-muted-foreground">
                    Cochez les critères utilisés pour regrouper les médicaments similaires. Au moins un doit rester actif.
                </p>
                <div class="flex flex-wrap gap-3">
                    <label
                        ...
                        :class="criteresActifs.includes(cle) ? 'border-[#459cd1] bg-[#459cd1]/10' : 'border-input'"
                    >
                        <input
                            type="checkbox"
            ...
            <form class="flex flex-wrap items-center gap-4" @submit.prevent="filtrer('search', searchQuery)">
                ...
                <div class="flex items-center gap-2">
                    <select
                        ...
                        class="flex h-9 rounded-md border border-input bg-background px-3 py-1 text-sm"
                        @change="(e: Event) => filtrer('critere', (e.target as HTMLSelectElement).value)"
                    >
            ...
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                ...
                <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-800/50 dark:bg-emerald-900/20">
                    ...
                    <p class="text-sm text-emerald-800 dark:text-emerald-300">Fusions effectuées</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-white/5">
                    <p class="text-3xl font-bold text-slate-800 dark:text-slate-200">{{ stats.total_produits }}</p>
                    <p class="text-sm text-muted-foreground">Produits concernés</p>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                >
                    <div class="mb-4 flex flex-wrap items-start justify-between gap-4">
                        ...
                        >
                            ...
                            <p class="font-semibold text-[#459cd1]">
                                {{ g.total_si_fusion?.ventes ?? 0 }} ventes · {{ Number(g.total_si_fusion?.ca ?? 0).toLocaleString('fr-FR') }} xaf
                            </p>
                        </div>
                        <div
                            v-else
                    ...
                    <div class="mb-4 grid gap-4 sm:grid-cols-2">
                        ...
                        >
                            <div class="mb-2 flex items-center gap-2">
                                ...
                                <h4 class="font-semibold text-foreground">{{ designationComplete(m) }}</h4>
                                <span
                                    v-if="m.is_principal"
                            ...
                            <p class="mb-1 text-sm text-muted-foreground">
                                Forme : {{ m.forme || '—' }} · Type : {{ m.type || '—' }}
                            </p>
                            <p class="mb-1 text-sm font-medium text-emerald-600">
                                {{ m.ventes }} ventes · {{ Number(m.ca).toLocaleString('fr-FR') }} xaf
                            </p>
                            <p class="text-xs text-muted-foreground">
                                PU : {{ m.pu != null ? Number(m.pu).toLocaleString('fr-FR') : '—' }} xaf · Créé le {{ m.created_at || '—' }}
                            </p>
                        </div>
                    </div>
                    ...
                    >
                        Groupe ignoré – ces médicaments sont considérés comme distincts
                    </div>
                    <div
            ...
            >
                <DialogHeader class="shrink-0 border-b px-4 py-3 pr-12 text-left sm:px-5">
                    <DialogTitle class="flex items-center gap-2 text-base font-semibold sm:text-lg">
                        <Package class="size-5 shrink-0 text-[#459cd1] sm:size-6" />
                ...
                <div class="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain px-4 py-3 sm:px-5">
                    <div class="flex gap-2 rounded-lg border border-amber-300 bg-amber-50 p-2.5 dark:border-amber-700 dark:bg-amber-900/20">
                        ...
                        <div class="text-xs leading-snug sm:text-sm">
                            ...
                            <p class="mt-0.5 text-amber-700 dark:text-amber-300">
                                Un seul médicament sera conservé. Les autres seront supprimés de la base. Choisissez celui à conserver.
                            </p>
                        </div>
                    </div>
                        ...
                        <div class="space-y-2">
                            ...
                            >
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="font-semibold text-foreground">{{ designationComplete(m) }}</p>
                                    <p class="text-xs text-muted-foreground">
                                        {{ m.forme || '—' }} · {{ m.ventes }} ventes · {{ Number(m.ca).toLocaleString('fr-FR') }} xaf
                                    </p>
                                </div>
                                <CheckCheck
                    ...
                    <div class="rounded-lg border border-slate-200 bg-slate-50 p-2.5 dark:border-slate-700 dark:bg-slate-800/50">
                        <p class="text-xs text-muted-foreground">
                            <strong>{{ (groupeEnFusion?.medicaments?.length ?? 1) - 1 }}</strong>
        ...
        <ConfirmModal
            ...
            :description="groupeToIgnorer ? `Marquer le groupe #${groupeToIgnorer.numero} comme ignoré ? Ces médicaments seront considérés comme distincts.` : ''"
